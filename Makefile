...
.PHONY: gen up down nuke build models index sync smoke logs ps status watch activity demo test-env k8s-apply k8s-status k8s-watch source-check
...
ps:      ; $(COMPOSE) ps
# is it working / is it progressing (services, documents processed per scope, code graph, last sync); make watch keeps it refreshing
status:  ; scripts/status.sh $(ARGS)
# same screen, refreshing every 5 s until Ctrl-C (ARGS="--interval 15" to slow it down)
watch:   ; scripts/watch.sh $(ARGS)
...
# live trail of gateway tool calls + engine activity while an agent talks to the stack (ARGS=--k8s for a cluster)
activity: ; python3 scripts/activity.py $(ARGS)
# the demo: make demo ARGS=up | ready | watch | activity | "connect --as alice" | add-page | smoke | "down --volumes"  (docs/DEMO.md)
demo:    ; scripts/demo.sh $(ARGS)
...
k8s-watch:  ; scripts/watch.sh --k8s $(ARGS)
