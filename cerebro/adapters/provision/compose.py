...
config_path (cerebro.yaml), ready_timeout (300 s), docker_cli_image, and compose_files: extra compose files (paths
relative to the working directory) added with -f after the rendered one to EVERY docker compose command, the way an
operator adds an override: extra services (mocks, a proxy), bind mounts into the ingest, published ports. Relative
...
from ...provision.common import PORT_LABEL, labels_for, unit_ports_from_env, volume_key
...
class Adapter(Provisioner):
    ...
    def _load_ports(self) -> None:
        """Ports from $CEREBRO_UNIT_PORTS (gateway / ingest inside the stack), then from the rendered file's
        cerebro.io/port labels, so endpoint() works without ensure() first."""
    ...
    def render(self, units: list[UnitSpec], jobs: list[JobSpec]) -> dict[str, str]:
        ...
        files = " ".join(f"-f {f}" for f in [str(self.compose_file), *self.extra_files])
        header = ("# GENERATED by `cerebro provision render` (target compose) from cerebro.yaml -- do not edit.\n"
                  f"# Run with: docker compose -p {self.project} {files} --env-file {self.env_file} ...\n"
    ...
    def _scheduler(self, jobs: list[JobSpec], configs: dict[str, dict]) -> dict:
        files = " ".join(f"-f {shlex.quote(f)}" for f in [str(self.compose_file), *self.extra_files])
        base = (f"cd /workspace && docker compose -p {shlex.quote(self.project)} {files} "
