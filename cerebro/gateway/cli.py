...
gateway.host:gateway.port (identity.mode none forces 127.0.0.1 unless identity.allow_remote).
"""
...
def _serve(args) -> int:
    ...
    for note in gw.notes:
        log.warning("%s", note)
    log.info("serving MCP at http://%s:%s%s (resource %s)", gw.host, config.gateway.port, config.gateway.path, config.resource_id())
    uvicorn.run(gw.app, host=gw.host, port=config.gateway.port, log_level=args.log_level.lower())
    return 0
