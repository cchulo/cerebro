...
| What if the index misses or lags? | `query_docs` falls back to the live sources for the same scopes: by default the upstream the plugin declares, mcp-atlassian run as `mcp-confluence` (or the REST API), and returns refs to `live_fetch`; see [SOURCES.md](SOURCES.md#fallbacks-live-sources-and-mcp-upstreams). |
