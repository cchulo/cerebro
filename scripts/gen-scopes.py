...
def k8s():
    ...
    plugdirs = {}
    for kind in ("sources", "live"):
        d = out / f"plugins-{kind}"; d.mkdir()
        files = sorted((root / "plugins" / kind).glob("*.py")) + [root / "plugins/README.md"]
        for f in files:
            if f.exists():
                (d / f.name).write_text(f.read_text())
        plugdirs[kind] = ", ".join(f"plugins-{kind}/{f.name}" for f in files if f.exists())
...
configMapGenerator:
  ...
  - name: index-script
    files: [index-repo.sh]
  - name: ingest-plugins
    files: [SOURCE_PLUGIN_FILES]
  - name: gateway-plugins
    files: [LIVE_PLUGIN_FILES]
generatorOptions:
  disableNameSuffixHash: true
""".replace("SOURCE_PLUGIN_FILES", plugdirs["sources"]).replace("LIVE_PLUGIN_FILES", plugdirs["live"]))
