...
def test_check_lists_what_a_plugin_yields(ingest):
    ...
    assert "files: configured=True" in text and "- public/adr/0001.md" in text and "3 documents" in text
...
def test_cli_sync_and_check(tmp_path, docs_dir, capsys, monkeypatch):
    ...
    assert report["public/files"] == {"changed": 3, "removed": 0} and report["public"]["index"]["inserted"] == 3
